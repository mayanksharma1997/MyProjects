...
public class AVLTree<K extends Comparable<K>> extends BinaryTree<K> {
   ...
   public void addRoot(K data) throws Exception {
      ...
      setRoot(new AVLNode<K>(null, null, null, data));
      size++;
   }
   ...
   public void rebalance(AVLNode<K> node) {
      if (node == null) return;
      int oldHt = node.ht;
      if (node.isLeftHeavy() || node.isRightHeavy()) {  // rotation required
         ...
         else if (node.isLeftHeavy()) // double rotate at left
      ...
      int newHt = node.setHt();
      if (oldHt != newHt)  // the parent might have lost balance too 
         rebalance((AVLNode<K>) node.getParent());
   }
   ...
   protected AVLNode<K> recursiveFindNode(AVLNode<K> node, K target) {
      ...
      countComparisons++;
      if (comparison == 0) {countComparisons++; return node; }
